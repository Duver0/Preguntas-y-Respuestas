...
Aplicación de preguntas y respuestas construida con React, Vite, Tailwind CSS y Bun, lista para desplegarse gratis como sitio estático. Incluye preguntas curadas de desarrollo web, rachas con bonus, historial detallado y un ranking persistido en `localStorage`.
...
- 🚀 Configuración de Vite lista para hosting estático (`base` apuntando al nombre del repo).
...
├─ index.html              # Entrada Vite lista para hosting estático
├─ tailwind.config.js
...
Este repositorio incluye un workflow (`.github/workflows/deploy.yml`) que construye el sitio con Bun y publica automáticamente el contenido optimizado como página estática.

1. En Settings → Pages selecciona **Source: GitHub Actions** y habilita la publicación.
2. Haz push a `main` (o ejecuta el workflow manualmente en la pestaña Actions). El flujo corre `bun run build`, copia `index.html` como `404.html` para manejar rutas internas y sube la carpeta `dist/` como artefacto listo para servirse.
3. Cuando el job **deploy** termine, la URL pública aparecerá en la sección Pages. Gracias al `base` de `vite.config.ts`, los assets se servirán correctamente desde la ruta generada.
